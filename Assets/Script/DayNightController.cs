...
public class DayNightController : MonoBehaviour
{
    ...
    //The amount of time remaining in the skybox transition
    ...
    /// <summary>
    /// The scene skybox material to use from Dusk To Day.
    /// </summary>
    public Material duskToDaySkybox;

    /// <summary>
    /// The scene skybox material to use from Dusk To Night.
    /// </summary>
    public Material duskToNightSkybox;

    /// <summary>
    /// The scene skybox material to use from Night To Dusk.
    /// </summary>
    public Material nightToDuskSkybox;

    /// <summary>
    /// The scene skybox material to use from Day To Dusk.
    /// </summary>
    public Material dayToDuskSkybox;
    ...
    private void UpdateSkybox()
    {
        if (remainingTransition > 0.0f)
        {
            if (currentPhase == DayPhase.Dawn)
            {
                RenderSettings.skybox = nightToDuskSkybox;
                nightToDuskSkybox.SetFloat("_Blend", 1.0f - remainingTransition / skyTransitionTime);
            }
            if (currentPhase == DayPhase.Day)
            {
                RenderSettings.skybox = duskToDaySkybox;
                duskToDaySkybox.SetFloat("_Blend", 1.0f - remainingTransition / skyTransitionTime);
            }
            if (currentPhase == DayPhase.Dusk)
            {
                RenderSettings.skybox = dayToDuskSkybox;
                dayToDuskSkybox.SetFloat("_Blend", 1.0f - remainingTransition / skyTransitionTime);
            }
            if (currentPhase == DayPhase.Night)
            {
                RenderSettings.skybox = duskToNightSkybox;
                duskToNightSkybox.SetFloat("_Blend", 1.0f - remainingTransition / skyTransitionTime);
            }
